...
`supply` drives), so no Ruby/fastlane needed. Reads:
    ...
    fastlane/metadata/android/<locale>/title.txt
                                       ...
                                       images/sevenInchScreenshots/*.png   (optional)
                                       images/tenInchScreenshots/*.png     (optional)
...
SHOT_TYPES = ('phoneScreenshots', 'sevenInchScreenshots', 'tenInchScreenshots')
...
def main():
    ...
    try:
        for loc in locales():
            ...
            # screenshots — phone plus the tablet slots (tablet-first app)
            for stype in SHOT_TYPES:
                shots = os.path.join(imgdir, stype)
        ...
        if args.commit:
            try:
                ...
                if e.resp.status == 400 and 'changesNotSentForReview' in str(
                    ...
                    raise
            print('[COMMIT] listing saved — publish/roll out in the Console')
        else:
