...
| Audio | **`audioplayers`** — low-latency mode for action-response sounds, a looping player for background music (concept §10). Sounds: Kenney **CC0**, bundled. Music: Kevin MacLeod **CC BY 4.0** (credited in About); the app's `assets/music/` mp3s **are tracked in git** — only the `examples/ui/music/` audition copies are gitignored (`examples/ui/README.md` has the re-fetch commands). Jukebox shipped 2026-07-12: per-track on/off switches in settings shrink the rotation pool (12 tracks @ 128 kbps ≈ 47 MB; per-level/per-board selection draws from the enabled ones) |
